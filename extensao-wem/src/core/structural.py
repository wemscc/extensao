...
def WoodPilarSupportLoad(width, length, weightRoofTiles, roofInclination, distanceRoofSupport, material):

        weightRoofTilesInKN = weightRoofTiles/100
        ...
        if length<=CollumnDistancing(material):
            InfluenceArea=(float(length)/2)*((width/2)*(cossin))
        else:
            InfluenceArea=float(distance/2)*(float(width/2)*(cossin))
        ...
        return (weightStructure+weightRoofTilesInKN)*InfluenceArea
 
def WoodKnotSupportLoad(width, length, weightRoofTiles, material):

        weightRoofTilesInKN = weightRoofTiles/100
    
        InfluenceArea = width*length
                
        weightStructure=material.GetMaterialWeight()*((constants.sessaoRipa/constants.espacamentoRipa)+(constants.sessaoCaibro/constants.espacamentoCaibro)+(constants.sessaoTerca/width))
        return (weightStructure+weightRoofTilesInKN)*InfluenceArea
...
def GetXDimention(load, material):
        ...
        if material.GetRelativeHumidity() <= 0.65:
            ...
            kmod2 = 0.8
        elif material.GetRelativeHumidity() > 0.85:
            kmod2 = 0.7
