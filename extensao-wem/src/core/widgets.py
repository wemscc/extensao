...
class WoodMaterialWidget(QWidget):
    ...
    def __init__(self, data = {}) -> None:
        ...
        self.__namesModel = QStringListModel(j for entry in self.__names for j in entry)
        ...
        # Load the attributes from the first  class
        att = [[value] for value in self.__attributes[self.__classComboBox.currentIndex()].values()]
        
        teste = [att for att in self.__attributes[self.__classComboBox.currentIndex()].values()]
        setData(teste)
        ...
        self.__classComboBox.currentIndexChanged.connect(self.__SearchByName)
    ...
    def __SelectClass(self):
        ...
        setData(classAttributes)

    def __SearchByName(self):
        name = self.__nameLineEdit.text()
        values = self.__names
        if name not in (item for sublist in values for item in sublist):
            return
        for sublist in values:            
            for value in sublist:
                if name == value:
                    indice = values.index(sublist)
        classAttributes = [att for att in self.__attributes[indice].values()]
        setData(classAttributes)
        for i, value in enumerate(classAttributes):
            self.__attributesViewModel.setData(self.__attributesViewModel.index(i, 0), value)
